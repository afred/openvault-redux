        ...
        _delegates: function () {
            ...
            self.$sausages
                ...
                .delegate('.sausage', 'click', function(e){
                  ...
                  //  self._scrollTo(o);
                    self.$outer.scrollTo(self.$inner.find(self.options.page).eq(val));
       ...
       /*     
        ...
        draw: function () {
            ...
            for (var i = 0; i < self.count; i++)
            {
                ...
                var height = 0;
                if(i+1 < self.count) {
                  height = $items.eq(i+1).position().top - $page.position().top;
                } else {
                  height = h_doc - $page.position().top;
                }
                offset_p = $page.position();
                offset_s = offset_p.top/h_doc*h_win;

                
                s.push('<div class="sausage' + ((i === self.current) ? ' sausage-current' : '') + '" style="height:' + (height/h_doc*h_win) + 'px;top:' + offset_s + 'px;">' + self.options.content(i, $page) + '</div>');
                
                // Create `self.offsets` for calculating current sausage.
                self.offsets.push(offset_p.top - slop);
            }
