module Openvault::DigitalObjects::Artesia
  module Asset
     ...
     def add_metadata_sdef_datastreams!
         ds = self['DublinCore']
         ds.dsLocation = "http://local.fedora.server/fedora/get/#{self.pid}/sdef:UOISMetadata/DublinCore"
         ds.mimeType = 'text/xml'
         ds.controlGroup = 'E'
         ds.save
     
         ds = self['PBCore']
         ds.dsLocation = "http://local.fedora.server/fedora/get/#{self.pid}/sdef:UOISMetadata/PBCore"
         ds.mimeType = 'text/xml'
         ds.controlGroup = 'E'
         ds.save
     ...
     def add_media_datastreams! files
         ...
         files.each do |f|
           ...
           mime = doc.xpath("//rdf:Description[@rdf:about='#{f}']/File:MIMEType", xmlns).first.text rescue 'File'
